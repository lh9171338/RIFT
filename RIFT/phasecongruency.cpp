...
void PhaseCongruency::calc(InputArray _src, vector<Mat> &_pc, vector<Mat>& _Ao)
{
    Mat src = _src.getMat();
...
void PhaseCongruency::feature(vector<Mat>& _pc, OutputArray _edges, OutputArray _corners)
{
    ...
    subtract(sum, denom, minMoment);//m = (covy2 + covx2 - denom) / 2;  % minimum moment
...
//Build up covariance data for every point
void PhaseCongruency::feature(InputArray _src, OutputArray _edges, OutputArray _corners)
{
    vector<Mat> pc;
	vector<Mat> Ao;
    calc(_src, pc, Ao);
